...
class UserJobListView(LoginRequiredMixin, ListView, FormGetMixin):
    ...
    def __form_filtered_queryset(self):
        ...
        tag_field = self.request.GET.get('tag', None)
    ...
    def get_source_facets(self):
        ...
        querys = self.__form_filtered_queryset()
        querys = querys.values('job__posts__source__name', 'job__posts__source__code').annotate(total=Count('job__posts__id', distinct=True)).order_by('job__posts__source__name')
        return querys
    ...
    def get_date_facets(self):
        ...
        if source:
            querys = querys.filter(job__posts__source__code=source)
        querys = querys.values(created_date=TruncDate('job__created')).annotate(total=Count('job_id', distinct=True)).order_by('created_date').reverse()
        return querys
